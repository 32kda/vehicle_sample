...
const MAX_ANGLE_DEG_FOR_VTARGET = 45
	...
	if state == State.ALIVE:
		if health_controller.is_destroyed():
			...
			if target != null:
				...
				if abs(rad_to_deg(angle)) <= MAX_ANGLE_DEG_FOR_VTARGET:
				...
				if rad_to_deg(full_angle) <= max_fire_angle:
					firing = true
					l_minigun.fire()
					r_minigun.fire()
			else:
